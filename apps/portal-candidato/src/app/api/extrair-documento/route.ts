...
import { extrairDocumento } from "@/lib/openai";
...
export async function POST(request: NextRequest) {
  ...
  try {
    const { resultado, modelo, versaoPrompt } = await extrairDocumento({
      bytesBase64,
    ...
    const { data: registrado, error: erroRegistro } = await supabase.schema("painel").rpc("registrar_extracao", {
      ...
      p_evidencias: resultado.trechos_evidencia,
    ...
    return NextResponse.json({ erro: e instanceof Error ? e.message : "Falha desconhecida na extração." }, { status: 502 });
  }
}
