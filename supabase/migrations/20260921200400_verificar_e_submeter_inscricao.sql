-- Fase 1 · Regras de negócio NO SERVIDOR: aceite das declarações, pendências e envio da inscrição.
-- O frontend só exibe; quem decide se a inscrição pode ser concluída é o banco (CLAUDE.md, princípio 5).
-- Reversão: supabase/rollback/20260921200400_verificar_e_submeter_inscricao.down.sql

------------------------------------------------------------------------------
-- Aceite das declarações (o horário é do servidor, o candidato não escolhe)
------------------------------------------------------------------------------
create or replace function publico.aceitar_declaracoes(p_versao text default 'edital-v2-2026-09-21')
returns void
...
begin
  ...
  if not publico.inscricoes_abertas() then
    raise exception 'Fora do período de inscrições.' using errcode = 'P0001', hint = 'inscricoes_fora_do_periodo';
  end if;
  update publico.inscricoes
     set declaracoes_aceitas_em = now(), declaracoes_versao = p_versao
   where id = v_id;
end;
$$;

------------------------------------------------------------------------------
-- Pendências da inscrição do usuário logado (uma linha por problema; vazio = pode enviar)
-- "etapa" é a etapa do formulário onde o candidato corrige.
-- Datas-limite (idade, laudo, colação) usam a DATA DE ENCERRAMENTO das inscrições (item 3.1).
------------------------------------------------------------------------------
...
begin
  ...
  if i.formato_diploma = 'digital' and nullif(btrim(i.codigo_diploma_digital), '') is null then
    return query select 'diploma_digital_sem_codigo'::text, 'Informe o código de autenticação ou o QR Code do diploma digital (item 5.1.2).'::text, 3;
  end if;
  ...
  if i.diploma_provisorio
    ...
    return query select 'historico_ausente'::text, 'Certificado provisório exige o histórico escolar com registro de colação de grau (item 5.1.4).'::text, 3;
  end if;
  if i.diploma_exterior then
    if not exists (select 1 from publico.documentos d where d.inscricao_id = i.id and d.ativo and d.tipo = 'revalidacao_diploma') then
      return query select 'revalidacao_ausente'::text, 'Diploma estrangeiro exige a revalidação (item 5.1.5).'::text, 3;
    end if;
    if not exists (select 1 from publico.documentos d where d.inscricao_id = i.id and d.ativo and d.tipo = 'traducao_juramentada') then
      return query select 'traducao_ausente'::text, 'Documento em língua estrangeira exige tradução juramentada (item 5.5.2).'::text, 3;
    end if;
  end if;
  ...
  for r in
    select v.cargo, v.empregador_contratante from publico.vinculos_declarados v
    where v.inscricao_id = i.id
      and not exists (select 1 from publico.documentos d where d.vinculo_id = v.id and d.ativo)
  loop
    return query select 'vinculo_sem_documento'::text,
      format('Anexe o documento que comprova o vínculo "%s" em %s.', r.cargo, r.empregador_contratante), 4;
  end loop;
  if i.nivel = 'senior'
    ...
    return query select 'lideranca_ausente'::text, 'Nível Sênior: anexe a declaração específica de liderança técnica (item 5.3.4).'::text, 4;
  end if;
  ...
  if i.cota_pcd then
    if i.data_laudo is null
      ...
      return query select 'laudo_ausente'::text, 'Informe a data e anexe o laudo médico (item 10.5).'::text, 5;
    elsif i.data_laudo < (enc - interval '12 months')::date or i.data_laudo > enc then
      return query select 'laudo_fora_do_prazo'::text, 'O laudo deve ter sido emitido em até 12 meses antes do encerramento das inscrições (item 10.5).'::text, 5;
    end if;
  end if;
  if i.cota_racial
  ...
  if i.solicitou_isencao
    ...
    return query select 'isencao_incompleta'::text, 'O pedido de isenção exige fundamentação e o requerimento anexado (item 4.10).'::text, 5;
  end if;

  -- 6 · pagamento (4.9.3: sem comprovante a inscrição não é concluída, exceto com pedido de isenção em análise)
  if not i.solicitou_isencao
    ...
    return query select 'comprovante_pix_ausente'::text, 'Anexe o comprovante do Pix (item 4.9.3).'::text, 6;
  end if;
...
------------------------------------------------------------------------------
-- Envio da inscrição: só se não houver nenhuma pendência. Pode ser repetido até o encerramento.
-- Com pedido de isenção o status é "aguardando_isencao" (4.10.2); a decisão não é tomada aqui.
------------------------------------------------------------------------------
...
begin
  ...
  select count(*) into v_pend from publico.verificar_inscricao();
  if v_pend > 0 then
...
revoke execute on function publico.aceitar_declaracoes(text), publico.verificar_inscricao(), publico.submeter_inscricao() from public, anon;
grant execute on function publico.aceitar_declaracoes(text), publico.verificar_inscricao(), publico.submeter_inscricao() to authenticated;
