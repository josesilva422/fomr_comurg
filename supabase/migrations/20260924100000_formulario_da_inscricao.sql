-- Aba "Formulário" da página do candidato no painel (pedido do responsável em 24/09/2026): mostra as respostas do
-- formulário organizadas em caixas. Reaproveita EXATAMENTE os mesmos dados do relatório em Excel/PDF: a montagem
-- do registro foi extraída para interno.registro_formulario() e as duas funções do painel passam a chamá-la.
-- Só Comissão. painel.formulario_da_inscricao NÃO grava auditoria (é a mesma visão da página do candidato);
-- o DOWNLOAD (painel.relatorio_respostas) continua gravando EXPORTAR_RELATORIO.
-- Reversão: supabase/rollback/20260924100000_formulario_da_inscricao.down.sql

create or replace function interno.registro_formulario(p_inscricao_id uuid)
returns jsonb
...
$$;
revoke execute on function interno.registro_formulario(uuid) from public, anon, authenticated;
...
create or replace function painel.formulario_da_inscricao(p_inscricao_id uuid)
returns jsonb
...
begin
  ...
  return interno.registro_formulario(p_inscricao_id);
end;
$$;
revoke execute on function painel.formulario_da_inscricao(uuid) from public, anon;
grant execute on function painel.formulario_da_inscricao(uuid) to authenticated;
